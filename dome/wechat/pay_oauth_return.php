<?php
use LSYS\PayGateway\Adapter\Wechat\PayWap;
...
<script>
    ...
    $(function(){
		...
		window.__LSYS\PayGateway=window.__LSYS\PayGateway||{};
		window.__LSYS\PayGateway.succ=function(url){
			show_page.succ(url);
			setTimeout(function(){
				window.location.href=url;
			},800);
		};
		window.__LSYS\PayGateway.fail=function(msg){
			dialog.show('支付失败:'+msg);
		}
		document.getElementById('wx_button').onclick=function(){
			window.__LSYS\PayGateway.wechat_obj.pay();	
		}
    });
</script>
</html>
